...
class LLMConfig(BaseModel):
    ...
    def resolve_api_key(self) -> Optional[str]:
        ...
        The .env file is loaded at CLI startup via python-dotenv,
        so all env vars (including those from .env) are available here.

        Resolution order:
        1. Explicit env var (api_key_env field)
        2. OPENROUTER_API_KEY env var as fallback for openrouter provider

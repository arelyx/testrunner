...
class TestLLMConfig:
    ...
    def test_resolve_api_key_fallback_openrouter(self):
        """Test fallback to OPENROUTER_API_KEY env var."""
        import os
        from unittest.mock import patch

        config = LLMConfig(provider="openrouter")
        with patch.dict(os.environ, {"OPENROUTER_API_KEY": "fallback-key"}):
            assert config.resolve_api_key() == "fallback-key"
    ...
    def test_resolve_api_key_strips_whitespace(self):
        ...
        config = LLMConfig(provider="openrouter", api_key_env="TEST_KEY_VAR")
        with patch.dict(os.environ, {"TEST_KEY_VAR": "  my-key  \n"}):
            assert config.resolve_api_key() == "my-key"

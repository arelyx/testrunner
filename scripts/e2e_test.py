...
3. Runs testrunner on each repository
...
def run_testrunner(repo_path: Path, config: dict) -> dict:
    """Run testrunner on a repository and return results.
    ...
    try:
        ...
        # Create runner
        runner = TestRunner(tr_config, db, repo_path, verbose=False)
        
        # Analyze git changes
        git_changes = {}
        if tr_config.git.enabled:
            try:
                git_changes = runner.analyze_git_changes()
            except Exception:
                pass
        
        # Analyze risks with LLM
        risk_scores = {}
        try:
            risk_scores = runner.analyze_risks()
        except Exception:
            pass
        ...
        try:
            ...
            report_path = str(report_gen.generate(results, runner.get_analysis_data()))
        except Exception:
            pass
        ...
        return {
            ...
            "failed_tests": [t.get("test_name", "") for t in results.get("failed_tests", [])],
...
def display_results(all_results: list[dict]) -> None:
    ...
    console.print()
    console.print("[bold]Risk Predictions vs Actual Failures:[/bold]")
    for result in all_results:
        if result.get("success") and result.get("risk_scores"):
            ...
            failed_set = set(result.get("failed_tests", []))
            
            for test_name, score in sorted_risks:
                marker = "[red]✗ FAILED[/red]" if test_name in failed_set else "[dim]passed[/dim]"
                console.print(f"  {score:.3f} - {test_name} {marker}")
